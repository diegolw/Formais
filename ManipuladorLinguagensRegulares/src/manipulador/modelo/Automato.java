...
public class Automato {
	...
	public void determinizar() {
		...
		for (String simbolo : alfabeto) {
			...
			EstadoAuxiliar _destino = (EstadoAuxiliar) _automato.getEstado(nome);
			if (_destino != null) {
		...
		// Dúvida! Os estados que contém o estado inicial original é também um estado inicial??
		
		for (Estado estado : _automato.getEstados()) {
			// Como esse automato é um automato de estados auxiliares, esse cast funcionará!
			EstadoAuxiliar auxiliar = (EstadoAuxiliar) estado;
			
			for (Estado incluso : auxiliar.getEstados()) {
				// Se incluso está contido na lista de finais, então esse é final também!
	...
	private Automato det(LinkedList<EstadoAuxiliar> estados, LinkedList<Transicao> transicoes, Automato _automato) {
		LinkedList<EstadoAuxiliar> novosEstados = new LinkedList<EstadoAuxiliar>();
